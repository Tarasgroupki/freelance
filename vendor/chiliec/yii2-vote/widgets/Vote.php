...
JS;
    ...
    public function run()
    {
		$likes = AggregateRating::find()->where(['model_id'=>$this->model->id])->andWhere(['likes' => 1])->count();
		$dislikes = AggregateRating::find()->where(['model_id'=>$this->model->id])->andWhere(['dislikes' => 1])->count();
		$rating = AggregateRating::find()->where(['model_id'=>$this->model->id])->one()['rating'];
        //print_r($model);
		return $this->render('vote', [
            'modelId' => $this->model->id,
            'targetId' => Yii::$app->user->id,
            'likes' => isset($likes) ? $likes : 0,
            'dislikes' => isset($dislikes) ? $dislikes : 0,
